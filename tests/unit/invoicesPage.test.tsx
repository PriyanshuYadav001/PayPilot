...
const m = vi.hoisted(() => {
  ...
  const customers: CustomerRow[] = [
    { id: 'c-1', organizationId: 'org-1', companyName: 'Globex Ltd', contactName: 'Jane Doe', email: 'jane@globex.com' },
    { id: 'c-2', organizationId: 'org-1', companyName: 'Initech', contactName: 'Bob Smith', email: 'bob@initech.com' },
  ];
  ...
  interface InvoiceRow {
    ...
    amountDue: number;
    status: string;
    items: Array<{ id: string; description: string; quantity: number; unitPrice: number; taxRate: number }>;
    createdAt: string;
    updatedAt: string;
  }
  ...
  const defaultInvoices: InvoiceRow[] = [
    {
      ...
      status: 'draft',
      items: [
        { id: 'it-1', description: 'Service', quantity: 2, unitPrice: 1000, taxRate: 18 },
        { id: 'it-2', description: 'Setup', quantity: 1, unitPrice: 500, taxRate: 0 },
      ],
    ...
    },
    {
      ...
      amountDue: 0,
      status: 'paid',
      items: [{ id: 'it-3', description: 'Retainer', quantity: 1, unitPrice: 5000, taxRate: 0 }],
  ...
  const apiRequest = vi.fn<(endpoint: string, options?: unknown) => Promise<unknown>>(
    async (endpoint: string, options?: unknown) => {
      ...
      if (url.pathname === '/customers') {
        return { success: true, data: { customers }, pagination: { page: 1, limit: 100, totalCount: customers.length, totalPages: 1 } };
      }

      if (url.pathname === '/invoices') {
        if (opts.method === 'POST') {
          const body = JSON.parse(opts.body ?? '{}');
          const subtotal = (body.items as Array<{ quantity: number; unitPrice: number }>).reduce(
            (s, i) => s + i.quantity * i.unitPrice,
            0
          );
          const taxTotal = (body.items as Array<{ quantity: number; unitPrice: number; taxRate?: number }>).reduce(
          ...
          const invoice: InvoiceRow = {
            ...
            status: 'draft',
            items: (body.items as Array<{ description: string; quantity: number; unitPrice: number; taxRate?: number }>).map(
              (item, index) => ({
                id: `it-created-${index}`,
        ...
        if (searchTerm) {
          ...
          rows = rows.filter((i) => i.invoiceNumber.toLowerCase().includes(term));
        }
        if (status) {
      ...
      if (url.pathname.startsWith('/invoices/')) {
        ...
        if (invoice) return { success: true, data: { invoice } };
...
describe('Invoices Page', () => {
  ...
  it('creates a new invoice through the modal', async () => {
    ...
    await act(async () => {
      ...
      fireEvent.change(screen.getByLabelText('Invoice number'), { target: { value: 'INV-2026-003' } });
      fireEvent.change(screen.getByLabelText('Issue date'), { target: { value: '2026-08-16' } });
      ...
      fireEvent.change(screen.getAllByLabelText('Item description')[0], { target: { value: 'Service' } });
      fireEvent.change(screen.getAllByLabelText('Quantity')[0], { target: { value: '2' } });
      fireEvent.change(screen.getAllByLabelText('Unit price')[0], { target: { value: '1000' } });
      fireEvent.change(screen.getAllByLabelText('Tax rate %')[0], { target: { value: '18' } });
    ...
    const postCall = m.apiRequest.mock.calls.find(
      (call) => call[0] === '/invoices' && (call[1] as { method?: string })?.method === 'POST'
    ) as [string, { method?: string; body?: string }] | undefined;
    const body = JSON.parse(postCall?.[1]?.body ?? '{}');
    expect(body.items[0]).toEqual(
      expect.objectContaining({ description: 'Service', quantity: 2, unitPrice: 1000, taxRate: 18 })
    );
    expect(await screen.findByText('INV-2026-003')).toBeInTheDocument();
  });
  ...
  it('deletes an invoice after confirmation', async () => {
    ...
    await act(async () => {
      fireEvent.click(screen.getByRole('button', { name: 'Delete INV-2026-001' }));
    });
    ...
    expect(m.apiRequest).toHaveBeenCalledWith(
      expect.stringMatching(/^\/invoices\/inv-1$/),
  ...
  it('filters invoices by search term', async () => {
    ...
    await act(async () => {
      fireEvent.change(screen.getByLabelText('Search invoices'), { target: { value: '002' } });
    });
